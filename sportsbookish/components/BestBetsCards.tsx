...
function Card({ row, direction, tournamentId, edgeField, isPaidTier }: { row: PlayerRow; direction: "buy" | "sell"; tournamentId: string; edgeField: keyof PlayerRow; isPaidTier: boolean }) {
  ...
  const PlayerName = isPaidTier ? (
    <Link href={`/golf/tournament/player?id=${tournamentId}&player_id=${row.player_id}`} className="text-sm font-semibold text-foreground hover:text-emerald-400 hover:underline truncate" title={row.player?.name}>
      {row.player?.name}
    </Link>
  ) : (
    <div className="text-sm font-semibold text-foreground truncate" title={row.player?.name}>{row.player?.name}</div>
  );

  return (
    <div className={`bg-card border border-border rounded-lg p-3 ring-1 ${ringCls}`}>
      ...
      <div className="grid grid-cols-2 gap-1 text-xs">
        ...
        <div className="text-right tabular-nums text-foreground/80">{fmtPct(row.books_median)}</div>
        <div className="text-muted-foreground">DG model</div>
      ...
      {row.best_book_for_bet && (
        <div className="mt-2 pt-2 border-t border-border text-[11px] text-muted-foreground">
          ...
          <span className="text-foreground">{bookLabel(row.best_book_for_bet.book)}</span>{" "}
          <span className="tabular-nums text-foreground/80">{fmtAmerican(row.best_book_for_bet.price_american)}</span>
        </div>
      )}
    </div>
  );
...
export default function BestBetsCards({ players, marketType, tournamentId, isPaidTier, edgeField = "edge_vs_books_median", minBooks = 3, minEdge = 0.002 }: Props) {
  ...
  if (buys.length === 0 && sells.length === 0) {
    return (
      <div className="bg-muted/40 border border-border rounded-lg p-4 mb-5 text-sm text-muted-foreground">
      ...
      {buys.length > 0 && (
        <div>
          <div className="flex items-baseline gap-2 mb-2">
            ...
            <span className="text-xs text-muted-foreground">Kalshi cheaper than fair — buy YES</span>
          </div>
      ...
      {sells.length > 0 && (
        <div>
          <div className="flex items-baseline gap-2 mb-2">
            ...
            <span className="text-xs text-muted-foreground">Sell YES on Kalshi, or bet at the books instead</span>
          </div>
