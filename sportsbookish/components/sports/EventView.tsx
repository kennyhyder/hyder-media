...
// /sports/[league]/event/[id] route (the latter 308s to canonical when the
// slug is known; falls back to inline render when it isn't).
export default async function EventView({
  ...
  // Parse title "Game N: A at B" into away/home if possible
  ...
  // FAQ generation — pick the row with the largest absolute edge for "best side"
  const bestMarket = detail.markets.reduce<typeof detail.markets[number] | null>((acc, m) => {
    const e = m.edge_vs_books_median;
    if (e == null) return acc;
    if (!acc || Math.abs(e) > Math.abs(acc.edge_vs_books_median ?? 0)) return m;
    return acc;
  }, null);
  const faqItems = faqForEventPage({
    eventTitle: detail.event.title,
    startTimeISO: detail.event.start_time,
    league,
    leagueDisplayName: meta.display_name,
    bestSide: bestMarket?.contestant_label ?? null,
    bestEdgePct: bestMarket?.edge_vs_books_median ?? null,
  ...
  const ldData = [
    breadcrumbLd([
      ...
      { name: detail.event.title, url: canonicalPath },
    ]),
    sportsEventLd({
    ...
    <div className="min-h-screen">
      ...
      <main id="main" className="container mx-auto max-w-3xl px-4 py-8">
        ...
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          <ForceRefreshButton entityId={eventId} source="sports" league={league} tier={tier} isAnonymous={isAnonymous} />
        </div>
                      ...
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mb-3">
                        <span>
                          ...
                          {isAnonymous && edgeMed != null && (
                            <Link href={`/signup?next=${encodeURIComponent(canonicalPath)}`} className="ml-1 text-[10px] text-amber-500 hover:text-amber-400 italic" title="See edge after Kalshi trading fees">
                              · 🔒 net-of-fee
                            </Link>
                          )}
                        </span>
                      ...
                      {books.length > 0 && (
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-1 text-[11px]">
                          {books.map((b, idx) => {
                            ...
                            const href = isAnonymous ? `/signup?next=${encodeURIComponent(canonicalPath)}` : "/pricing";
                            if (locked) {
                              return (
                          ...
                          {!isPaidTier && books.length > 5 && (
                            <Link href={isAnonymous ? `/signup?next=${encodeURIComponent(canonicalPath)}` : "/pricing"} className="flex items-center justify-center bg-amber-500/10 text-amber-500 hover:bg-amber-500/20 rounded px-2 py-1 col-span-full sm:col-span-1 text-[10px] font-semibold">
        ...
        {detail.spreads && detail.spreads.length > 0 && (
          <div className="mb-4">
            <SpreadsTable rows={detail.spreads} isPaidTier={isPaidTier && !isAnonymous} signupHref={isAnonymous ? `/signup?next=${encodeURIComponent(canonicalPath)}` : undefined} />
          </div>
        )}
        ...
        <FaqSection items={faqItems} heading={`${detail.event.title} odds — FAQ`} />
      </main>
    </div>
  );
}
