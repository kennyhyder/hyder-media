    ...
    <div className="min-h-screen">
      ...
      <main className="container mx-auto max-w-[1800px] px-4 py-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-5">
          <Card><CardContent className="p-4"><div className="text-xs uppercase text-neutral-500">Players</div><div className="text-2xl font-bold">{rows.length}</div></CardContent></Card>
          <Card><CardContent className="p-4"><div className="text-xs uppercase text-neutral-500">With Kalshi data</div><div className="text-2xl font-bold text-amber-400">{kalshiCount}</div></CardContent></Card>
          <Card><CardContent className="p-4"><div className="text-xs uppercase text-neutral-500">Consistency issues</div><div className={`text-2xl font-bold ${issuesCount > 0 ? "text-rose-400" : "text-emerald-400"}`}>{issuesCount}</div></CardContent></Card>
        </div>

        <Card className="overflow-hidden">
          <CardHeader>
            <CardTitle className="text-sm font-normal text-neutral-400">Each row is one player. Each market type shows three probability values: <span className="text-amber-400">K</span>alshi · <span className="text-sky-400">DG</span> · <span className="text-neutral-300">Bk</span> (book median). Win ≤ T5 ≤ T10 ≤ T20 should always hold; violations flagged in Issues.</CardTitle>
          </CardHeader>
          <CardContent className="p-0 overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-neutral-900/50">
                <tr>
                  <th rowSpan={2} className="px-3 py-2 text-left text-xs uppercase tracking-wide text-neutral-500 border-r border-border/40">Player</th>
                  {TYPES.map((mt) => (
                    <th key={mt} colSpan={3} className="px-2 py-1 text-center text-xs uppercase tracking-wide text-neutral-300 border-r border-border/40">{MARKET_LABELS[mt]}</th>
                  ))}
                  <th rowSpan={2} className="px-3 py-2 text-right text-xs uppercase tracking-wide text-neutral-500">Issues</th>
                </tr>
                <tr className="text-[10px] uppercase tracking-wide text-neutral-500">
                  {TYPES.map((mt) => (
                    <>
                      ...
                      <th key={`${mt}-b`} className="px-1 py-1 text-right text-neutral-400 border-r border-border/40">Bk</th>
                    </>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-border/40">
                {rows.map((r) => (
                  <tr key={r.player_id} className="hover:bg-neutral-900/40">
                    ...
                    {TYPES.map((mt) => {
                      ...
                      return (
                        <>
                          ...
                          <td key={`${r.player_id}-${mt}-b`} className="px-1 py-1 text-right tabular-nums text-neutral-400 border-r border-border/40">{fmtPct(e?.books_median_p, 2)}</td>
                        </>
                      );
                    })}
                ...
                {rows.length === 0 && (
                  <tr><td colSpan={17} className="px-3 py-8 text-center text-neutral-500">No ladder data — Kalshi may not yet have T5/T10/T20 markets posted for this event.</td></tr>
                )}
              </tbody>
            </table>
          </CardContent>
