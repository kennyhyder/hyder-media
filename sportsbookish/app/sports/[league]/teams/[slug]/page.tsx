...
import { eventUrl, teamUrl } from "@/lib/slug";
...
export async function generateMetadata({ params }: {
  ...
  const leagueName = leagueMeta?.display_name || league.toUpperCase();
  ...
  const description = `Live Kalshi odds for ${t.team.name} across every market — ${t.counts.games} ${t.counts.games === 1 ? "game" : "games"} and ${t.counts.futures} futures (championship, division, awards). Compare against DraftKings, FanDuel, BetMGM and 8+ books.`;
  return {
    title,
    description,
...
export default async function TeamPage({ params }: {
  ...
  // Build SportsTeam + ItemList JSON-LD
  const ldData: object[] = [
    breadcrumbLd([
      { name: "Home", url: "/" },
      { name: "Sports", url: "/sports" },
      { name: leagueMeta.display_name, url: `/sports/${league}` },
      { name: t.team.name, url: teamUrl(league, slug) },
    ]),
    {
      "@context": "https://schema.org",
      "@type": "SportsTeam",
      name: t.team.name,
      url: `${SITE_URL}${teamUrl(league, slug)}`,
      ...
      <main id="main" className="container mx-auto max-w-[1400px] px-4 py-8">
        <div className="mb-6">
          ...
          <p className="text-sm text-muted-foreground">
            ...
            <span className="ml-2 text-foreground/80">{t.counts.games} games · {t.counts.futures} futures</span>
          </p>
        </div>
