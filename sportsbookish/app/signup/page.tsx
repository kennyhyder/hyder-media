...
function SignupInner() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const redirectTo = `${window.location.origin}/auth/callback?next=${encodeURIComponent(next)}${tier ? `&tier=${tier}` : ""}`;
      ...
      <main className="container mx-auto max-w-md px-4 py-16">
        <Card>
          <CardHeader className="text-center">
            ...
            <CardTitle className="text-2xl">{sent ? "Check your email" : "Create your account"}</CardTitle>
            <CardDescription>
              {sent ? (
                <>We sent a magic link to <strong>{email}</strong>. Click it to log in — no password needed.</>
              ) : (
                <>Free forever for the winner lines. Upgrade anytime.</>
              )}
            </CardDescription>
          </CardHeader>
          <CardContent>
            {sent ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <p className="text-xs text-muted-foreground text-center">
                  ...
                  <Link href={`/login${next !== "/dashboard" ? `?next=${encodeURIComponent(next)}` : ""}`} className="text-emerald-400 hover:underline">
                    Log in
                  </Link>
                </p>
              </form>
