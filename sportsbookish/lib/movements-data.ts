...
export async function fetchContestant(id: string): Promise<ContestantData | null> {
  const r = await fetch(`${DATA_HOST}/api/sports/contestant?id=${id}`, { next: { revalidate: 15 } });
  if (!r.ok) return null;
  return r.json();
}
...
export async function fetchEventHistory(eventId: string, hours = 24): Promise<MarketHistory[]> {
  const r = await fetch(`${DATA_HOST}/api/sports/event-history?id=${eventId}&hours=${hours}`, { next: { revalidate: 15 } });
  if (!r.ok) return [];
  const data = await r.json();
  return data.markets || [];
}
