...
export async function fetchProps(tournamentId: string): Promise<PropEvent[]> {
  const r = await fetch(`${DATA_HOST}/api/golfodds/props?tournament_id=${tournamentId}`, { next: { revalidate: 30 } });
  if (!r.ok) return [];
...
export async function fetchLadder(tournamentId: string): Promise<LadderRow[]> {
  const r = await fetch(`${DATA_HOST}/api/golfodds/ladder?tournament_id=${tournamentId}`, { next: { revalidate: 30 } });
  if (!r.ok) return [];
  const data = await r.json();
  return data.players || [];
}
