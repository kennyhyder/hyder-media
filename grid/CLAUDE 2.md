...
A scored database of **~164,000 datacenter candidate sites** across the United States, each rated 0-100 on DC Readiness based on 10 weighted factors. Built on top of a nationwide transmission infrastructure foundation (52K+ lines, 38K+ substations).

### Nine Site Types (~164K total)

| Type | Count | Source |
|------|------:|--------|
| **Greenfield** | 43,225 | Sampled along 230+ kV transmission corridors every 15 km |
| **Industrial** | 35,788 | EPA ACRES brownfield/industrial sites |
| **Substation** | 29,891 | Existing substations >= 69 kV |
| **Mine** | 26,205 | MSHA mine sites (retired/inactive) |
| **Federal Excess** | 17,528 | GSA FRPP excess federal properties |
| **Manufacturing** | 7,657 | HIFLD manufacturing facilities |
| **Shovel Ready** | 2,365 | State economic development sites |
| **Brownfield** | 1,413 | EIA retired plants + EPA RE-Powering |
| **Military BRAC** | 26 | Base Realignment and Closure sites |

### Infrastructure Foundation
- 52,098 HIFLD transmission lines across all 50 states
- 38,701 substations (extracted from line endpoints)
- 3,731 datacenters (OSM + PNNL + Epoch AI)
- 1,381 IXP facilities (PeeringDB)
- 3,222 counties with risk/labor/climate/water/tax/fiber data
- BLM ROW grants, energy corridors, ERCOT congestion data, WECC path ratings
...
### Current Score Distribution (~164,000 sites, Mar 2026)

- **164,098 total scored sites** across 9 site types
- Top states by count: TX, CA, OH, PA, NC, NY, VA
- Greenfield sites generated by sampling points every 15 km along 230+ kV transmission lines with 5 km exclusion zone around existing sites
...
| # | Table | Records | Purpose | Source Script |
|---|-------|---------|---------|---------------|
| 9 | `grid_dc_sites` | ~164,098 | Scored DC candidate sites (9 site types) | `generate-dc-sites.py` + `generate-greenfield-sites.py` + `score-dc-sites.py` + industrial ingestion scripts |
...
| 13 | `grid_datacenters` | 3,731 | Existing US datacenter locations | `ingest-pnnl-dc.py` + `ingest-osm-datacenters.py` + `ingest-epoch-datacenters.py` |
...
## Scripts (84 Python + 1 Node.js)

> **Note**: Script count grew from 25 to 84 during Phase 2-4 expansion. Many are enrichment scripts that patch additional data onto existing records. See `grid/scripts/` for full list. Key additions since initial build:
>
> **Industrial site ingestion**: `ingest-epa-acres.py`, `ingest-gsa-frpp.py`, `ingest-msha-mines.py`, `ingest-brac-bases.py`, `ingest-hifld-manufacturing.py`, `ingest-shovel-ready.py`
>
> **Additional data layers**: `ingest-osm-datacenters.py`, `ingest-epoch-datacenters.py`, `ingest-datacentermap.py`, `ingest-ira-energy-communities.py`, `ingest-opportunity-zones.py`, `ingest-fiber-routes.py`, `ingest-railroad-fiber.py`, `ingest-dot-fiber.py`, `ingest-eia860m-proposed.py`, `ingest-eia861-rates.py`, `ingest-fra-rail.py`
>
> **Enrichment scripts**: `enrich-broadbandnow.py`, `enrich-cloud-regions.py`, `enrich-congestion-score.py`, `enrich-construction-cost.py`, `enrich-energy-prices.py`, `enrich-environmental.py`, `enrich-fcc-broadband.py`, `enrich-fema-flood.py`, `enrich-ferc714-load.py`, `enrich-gas-pipelines.py`, `enrich-iso-lmp.py`, `enrich-land-cover.py`, `enrich-land-pricing.py`, `enrich-nrel-rates.py`, `enrich-osm-fiber.py`, `enrich-osm-landuse.py`, `enrich-padus-land.py`, `enrich-parcel-owners.py`, `enrich-reserve-margin.py`, `enrich-tax-incentives.py`, `enrich-usgs-water.py`, `enrich-utility-contacts.py`, `enrich-water-systems.py`, `enrich-wri-water.py`, `enrich-hunter-contacts.py`, `enrich-peeringdb-contacts.py`, `enrich-land-contacts.py`, `enrich-eia-brownfield-contacts.py`
>
> **Utility scripts**: `update-all.py` (automated update orchestration), `compute-queue-metrics.py`, `geocode-brownfields.py`, `join-parcel-owners.py`, `scrape-website-contacts.py`
...
## API Endpoints (21 Vercel Serverless Functions)
...
| Endpoint | Purpose |
|----------|---------|
| `_demo.js` | Demo access token validation (rate-limited) |
| `_utils.js` | Shared utilities |
| `demo-validate.js` | Demo token validation endpoint |
...
| `GET /api/grid/corridor` | Single corridor detail | id | 30s |
| `GET /api/grid/substations` | Substation search | state, min_voltage, search | 30s |

### DC Site Selection APIs
...
#### `GET /api/grid/brownfield` — Single Brownfield Detail
- **Params**: `id` (required)
- **Timeout**: 30s

#### `GET /api/grid/county-heat` — Heat Map Data
- Returns county-level DC readiness scores (3,222 county centroids) for heat map overlay
- Combines site averages + proxy scores from county_data fields
- **Cache**: 1hr public, 2hr stale-while-revalidate
- **Timeout**: 30s

#### `GET /api/grid/map-data` — Map Marker Data
- Returns site data optimized for map rendering
- **Timeout**: 30s

#### `GET /api/grid/hyperscale` — Hyperscale Datacenter Data
- Hyperscale datacenter tracking data
- **Timeout**: 30s

#### `GET /api/grid/queue-overlay` — Queue Overlay Data
- ISO interconnection queue visualization data
- **Timeout**: 30s

## Web Interface (Next.js + Tailwind)

16 pages built and deployed as static export.

### Navigation
`Dashboard | Greenfields | Industrial | Map | Lines | Corridors | Market | Compare | Hyperscale`

### Pages

| Page | Route | File | Features |
|------|-------|------|----------|
| **Dashboard** | `/grid/` | `page.tsx` | National hero stats, top sites table, score distribution chart, state averages |
| **Greenfields (DC Sites)** | `/grid/sites/` | `sites/page.tsx` | Filters (search, state, type, min score), sortable table, score color-coding, pagination, CSV export, map toggle |
| **Industrial (Brownfields)** | `/grid/brownfields/` | `brownfields/page.tsx` | Industrial site filters, sortable table, map toggle with geo-filter |
| **Site Detail** | `/grid/site/?id=X` | `site/page.tsx` | Score breakdown bars (10 sub-scores with weights), power/connectivity/risk sections, due diligence checklist, satellite map |
| **Brownfield Detail** | `/grid/brownfield/?id=X` | `brownfield/page.tsx` | Brownfield-specific detail page |
| **Interactive Map** | `/grid/map/` | `map/page.tsx` | Full-screen Leaflet map with clustered markers, heat map overlay (county-level infrastructure suitability), transmission line overlay, location search (Nominatim geocoding), custom scoring weight controls |
| **Market Analysis** | `/grid/market/` | `market/page.tsx` | State comparison table, IXP density, scoring methodology |
| **Compare** | `/grid/compare/` | `compare/page.tsx` | Portfolio analysis / site comparison tool |
| **Hyperscale** | `/grid/hyperscale/` | `hyperscale/page.tsx` | Hyperscale datacenter tracking |
| **Lines/Search** | `/grid/search/` | `search/page.tsx` | Transmission line search with TransmissionMap |
| **Line Detail** | `/grid/line/?id=X` | `line/page.tsx` | Full line properties, ERCOT data, TransmissionMap |
| **Lines List** | `/grid/lines/` | `lines/page.tsx` | Transmission lines list |
| **Corridors** | `/grid/corridors/` | `corridors/page.tsx` | Energy corridor search |
| **Corridor Detail** | `/grid/corridor/?id=X` | `corridor/page.tsx` | Corridor detail page |
| **Parcels** | `/grid/parcels/` | `parcels/page.tsx` | Adjacent land parcels |
| **API Docs** | `/grid/api-docs/` | `api-docs/page.tsx` | API documentation |

### Key Components

- **TransmissionMap** (`src/components/TransmissionMap.tsx`) - Leaflet map rendering WKT polylines. Color-coded by capacity. Satellite imagery default for site detail, context-aware layer selection.
- **ResultsMap** (`src/components/ResultsMap.tsx`) - Leaflet map for site search results with clustered markers
- **HeatMapLayer** (`src/components/HeatMapLayer.tsx`) - County-level infrastructure suitability heat map using leaflet.heat (3,222 county centroids). Has zoom-level visibility issues at high zoom (see memory/gridscout-heatmap-followup.md).
- **GeoSearch** (`src/components/GeoSearch.tsx`) - Location search with Nominatim geocoding + radius filter
- **LocationFilter** (`src/components/LocationFilter.tsx`) - Geographic filtering component
- **WeightEditor** (`src/components/WeightEditor.tsx`) - Custom scoring weight controls for interactive map
- **NavBar** (`src/components/NavBar.tsx`) - Navigation bar component
- **DemoBanner/DemoWrapper** (`src/components/Demo*.tsx`) - Demo access system with rate-limited tokens
- **ErrorBoundary** (`src/components/ErrorBoundary.tsx`) - React error boundary
- **Layout** (`src/app/layout.tsx`) - Purple accent (#7c3aed), lightning bolt SVG icon. Metadata: "Datacenter site selection intelligence."
- **Suspense boundaries** - Pages with `useSearchParams()` wrap in `<Suspense>` (required for Next.js 16 static export)
...
| Geographic scope | All US | All US (50 states, 38K+ substations, ~164K scored sites) |
...
| Scored entities | — | ~164,000 sites (0-100 DC Readiness Score, 9 site types) |

## Demo Access System

GridScout uses a demo token system for sharing with prospects:
- **Demo token**: `GRIDSCOUT` (configured in `api/grid/_demo.js`)
- Rate-limited: tokens validated via `demo-validate.js` endpoint
- `DemoBanner.tsx` and `DemoWrapper.tsx` components handle the demo UI
- All API endpoints call `checkDemoAccess()` before serving data

## Presentation Deck

- **Location**: `/decks/gridscout/index.html`
- **URL**: https://hyder.me/decks/gridscout/
- 10-slide HTML deck with CSS/SVG animated mockups of the GridScout interface
- Purple theme matching the app, keyboard/touch/swipe navigation
- Animated demos: interactive map, DC readiness score, search & filter, site detail
- `CAPTURE-GUIDE.md` has instructions for creating GIF captures of the live site
