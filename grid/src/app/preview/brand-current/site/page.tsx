...
import { C, display, mono } from "@/components/preview/brand-current/theme";
import { Shell, TopBar } from "@/components/preview/brand-current/Shell";
import { Card, Chip, Label, ArcScore, Stat, SubScore } from "@/components/preview/brand-current/ui";

export const metadata: Metadata = {
  title: "Current — Site Profile",
...
export default async function CurrentSite() {
  ...
  if (!site) {
    return (
      <Shell>
        <TopBar active="site" />
        <p style={{ padding: 48, color: C.muted }}>No site data available.</p>
      </Shell>
    );
  }
  ...
  return (
    <Shell>
      ...
      <div style={{ maxWidth: 1120, margin: "0 auto", padding: "30px 28px 56px" }}>
        {/* breadcrumb */}
        <nav style={{ fontFamily: mono, fontSize: 11.5, color: C.muted, marginBottom: 28, display: "flex", gap: 8 }}>
          <Link href="/preview/brand-current" style={{ color: C.accentSoft, textDecoration: "none" }}>
            Explore
          </Link>
          <span>/</span>
          ...
          <span>/</span>
          <span style={{ color: C.text }}>{titleCase(site.name)}</span>
        </nav>
        ...
        >
          <div>
            <div style={{ display: "flex", gap: 8, marginBottom: 18, flexWrap: "wrap" }}>
              ...
              {site.substation_voltage_kv ? <Chip>{site.substation_voltage_kv} kV</Chip> : null}
            </div>
            <h1
              style={{
                fontFamily: display,
          ...
          <div style={{ justifySelf: "center" }}>
            <ArcScore score={site.dc_score} size={196} />
          </div>
        </div>
        ...
        <Card style={{ marginTop: 30 }} pad={24}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "18px 48px" }} className="cur-sub">
            {subScores.map(([l, v]) => (
        ...
        {/* ── DETAIL CARDS ──────────────────────────────────── */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0,1fr))", gap: 20, marginTop: 20 }} className="cur-2col">
          <Card>
            <Label style={{ color: C.accentSoft, marginBottom: 10 }}>Power &amp; interconnection</Label>
          ...
          <Card>
            <Label style={{ color: C.accentSoft, marginBottom: 10 }}>Speed-to-power</Label>
          ...
          <Card>
            <Label style={{ color: C.accentSoft, marginBottom: 10 }}>Connectivity</Label>
          ...
          <Card>
            <Label style={{ color: C.accentSoft, marginBottom: 10 }}>Risk &amp; environment</Label>
        ...
        <p style={{ color: C.muted, fontSize: 12.5, lineHeight: 1.6, marginTop: 30, paddingTop: 20, borderTop: `1px solid ${C.border}` }}>
      ...
      <style>{`
        @media (max-width: 860px) {
          .cur-hero { grid-template-columns: 1fr !important; }
          .cur-2col { grid-template-columns: 1fr !important; }
          .cur-sub { grid-template-columns: 1fr !important; }
        }
      `}</style>
    </Shell>
  );
...
function HeroMetric({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <div style={{ fontFamily: mono, textTransform: "uppercase", letterSpacing: "0.1em", fontSize: 10, color: C.muted, marginBottom: 6 }}>
        {label}
      </div>
      <div style={{ fontFamily: mono, fontVariantNumeric: "tabular-nums", fontSize: 21, fontWeight: 600, color: C.text }}>
        {value}
      </div>
    </div>
  );
