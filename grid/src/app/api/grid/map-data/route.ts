...
import { checkDemoAccess, isAnonRequest } from "@/lib/grid-api/demo";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const include_dcs = searchParams.get("include_dcs");
    const include_ixps = searchParams.get("include_ixps");
    const include_lines = searchParams.get("include_lines");
    const include_substations = searchParams.get("include_substations");
    const include_fiber = searchParams.get("include_fiber");
    const bounds = searchParams.get("bounds"); // "sw_lat,sw_lng,ne_lat,ne_lng"
    const lite = searchParams.get("lite"); // "1" for minimal columns (faster initial load)
    const limit = searchParams.get("limit"); // max sites to return (default 5000)
    ...
    // C2: cap anonymous (no-token, full-mode) callers hard to blunt bulk scraping.
    // Demo-token and authenticated full-access callers keep the higher ceiling.
    const isAnon = isAnonRequest(access, searchParams, request);
    const limitCeiling = isAnon ? 2000 : 20000;
    const maxSites = Math.min(Math.max(parseInt(limit || "") || 5000, 1), limitCeiling);

    // Parse bounds ONCE and reuse everywhere
    let parsedBounds: { swLat: number; swLng: number; neLat: number; neLng: number } | null = null;
    if (bounds) {
      ...
      if (!parsedBounds) {
        return handleError("bounds must be sw_lat,sw_lng,ne_lat,ne_lng with valid coordinates", 400);
      }
    }
    ...
    while (allSites.length < maxSites) {
      ...
      if (error) { console.error("Map data sites query error:", error.message); return internalError(); }
    ...
    const out: Record<string, unknown> = { sites: allSites, total: totalCount, returned: allSites.length };
    ...
    if (include_dcs === "true" || include_dcs === "1") {
      ...
      const { data: dcs, error: dcErr } = await dcQuery
        ...
        .limit(1000);
      if (dcErr) { console.error("Map data datacenters query error:", dcErr.message); return internalError(); }
      out.datacenters = dcs || [];
    }
    ...
    if (include_ixps === "true" || include_ixps === "1") {
      ...
      const { data: ixps, error: ixpErr } = await ixpQuery
        ...
        .limit(1000);
      if (ixpErr) { console.error("Map data ixps query error:", ixpErr.message); return internalError(); }
      out.ixps = ixps || [];
    }
    ...
    if (include_substations === "true" || include_substations === "1") {
      ...
      const { data: subs, error: subErr } = await subQuery
        ...
        .limit(2000);
      if (subErr) { console.error("Map data substations query error:", subErr.message); return internalError(); }
